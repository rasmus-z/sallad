  ...
  if (error) {
    return (
      <div className="mt-6 space-y-3">
        <p className="text-[13px] leading-relaxed text-danger/80">{error}</p>
        <button
          type="button"
          onClick={() => void load()}
          className={cn(
            "w-full border border-white/15 bg-white/10 py-3 text-[14px] font-medium text-white transition hover:bg-white/15",
            radius.md,
          )}
        >
...
export function OpenRouterSetupStep({ onExitBack }: { onExitBack: () => void }) {
  ...
  const backButton = (
    <button
      type="button"
      onClick={goPrev}
      className="mt-4 flex items-center gap-1.5 self-center text-[13px] font-medium text-white/45 transition hover:text-white/70"
    >
      <ArrowLeft size={14} />
